...
eval_gnn_rct.py
---------------
Evaluate the RCT fine-tuned GNN consistency model.
Tests 1-step generation (the key result) and multi-step for comparison.

Usage:
    python eval_gnn_rct.py \
        --checkpoint runs-gnn-rct3/best-model.pkl
"""
...
def sample_1step(net, node_feat_base, edge_index, edge_attr, device):
    """1-step: start from pure noise at t=1, predict x1 directly."""
    ...
    return net(node_t, edge_index, edge_attr)  # (N, 4)


@torch.no_grad()
def sample_nstep(net, node_feat_base, edge_index, edge_attr, device, n_steps=5):
    """
    Multi-step consistency sampling.
    At each step, predict x1, then re-noise to next timestep and repeat.
    """
    N  = node_feat_base.shape[0]
    ts = torch.linspace(1.0, 0.05, n_steps + 1, device=device)

    # Start from pure noise
    x = torch.rand(N, 4, device=device) * 2*math.pi

    for i in range(n_steps):
        t_val  = ts[i]
        t_feat = torch.full((N, 1), t_val.item(), device=device)
        node_t = torch.cat([node_feat_base, x, t_feat], dim=-1)

        # Predict clean x1
        x1_pred = net(node_t, edge_index, edge_attr)  # (N, 4)

        if i < n_steps - 1:
            # Re-noise to next timestep
            t_next = ts[i+1]
            log_x_x1 = torch.atan2(
                torch.sin(x1_pred - x), torch.cos(x1_pred - x)
            )
            x = (x + t_next * log_x_x1) % (2*math.pi)

    return x1_pred  # (N, 4)


def evaluate(net, backbone_path, side_chain_path, sequence_path,
             device, n_steps_list=[1, 2, 5], test_split=0.1, seed=42):

    bb_data  = np.load(backbone_path)
    sc_data  = np.load(side_chain_path)
    seq_data = np.load(sequence_path, allow_pickle=True)

    backbone = torch.tensor(bb_data['backbone'], dtype=torch.float32)
    bb_mask  = torch.tensor(bb_data['mask'],     dtype=torch.float32)
    chi_gt   = torch.tensor(sc_data['side_chains'], dtype=torch.float32)
    chi_mask_npz = torch.tensor(sc_data['mask'],    dtype=torch.float32)
    names    = seq_data['names']

    # Use angle-nonzero as true per-chi validity
    ...
    print(f"Evaluating on {n_test} test proteins...")

    net.eval()

    def run_eval(n_steps, label):
        ...
        for ii, idx in enumerate(test_idx):
            ...
            if n_steps == 1:
                ...
                pred = sample_nstep(net, node_feat_base, edge_index, edge_attr,
        ...
        print(f"\nPer-chi:")
        print(f"  {'Chi':<8} {'MAE (°)':<12} {'Acc (%)':<12} {'N valid'}")
        print(f"  {'-'*48}")
    ...
    for n in n_steps_list:
        label = f"{n}-step consistency sampling"
        run_eval(n, label)
...
def main():
    ...
    print(f"Loaded. Epoch: {data.get('epoch','?')} | "
          f"Stored MAE: {data.get('test_mae','?'):.2f}° | "
          f"Stored Acc: {data.get('test_acc','?'):.2f}%")
