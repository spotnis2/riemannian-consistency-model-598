...
class Trainer(object):
    ...
    def train(self, ts, resume=False):
        ...
        for epoch in trange(start_epoch, (self.config.train.num_epochs), desc = '[Epoch]', position = 1, leave=False):
            ...
            if not self.ddp or dist.get_rank() == 0:
                mean_test_chi = np.mean(test_loss_chi)
                mean_train_chi = np.mean(self.train_chi)

                writer.add_scalar("train_chi", mean_train_chi, epoch)
                writer.add_scalar("test_chi", mean_test_chi, epoch)
                writer.flush()

                # -------- Log losses --------
                logger.log(f'[EPOCH {epoch+1:04d}] | time: {time.time()-start_time:.2f} sec | '
                               f'train chi: {mean_train_chi:.3e}'
                               f'test chi: {mean_test_chi:.3e}', verbose=False)
