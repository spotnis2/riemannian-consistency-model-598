...
def filter_edges_by_residue_mask(edge_index, residue_mask):
    """Drop edges touching padded / invalid residues (``aa_mask == 0``)."""
    ...
    def __getitem__(self, idx):
        ...
        if self.filter_length:
            if len(coords) > self.max_length:
                ...
                atom_type = self.pad_tensor(atom_type, m)
                chain_id = self.pad_long_field(chain_id, m, fill_value=-1)
                res_id = self.pad_long_field(res_id, m, fill_value=-1)
                icode_s = np.asarray(icode, dtype=str)
                icode = np.concatenate([icode_s, np.full(m, '', dtype=str)])
