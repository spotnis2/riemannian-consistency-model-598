...
class MPModel(nn.Module):
    def __init__(self, in_channels=2, base_channels=128, x_channel_mult=[2, 4, 4, 2], emb_channel_mult=2, dropout=0.0, cond_dim=27):
        super().__init__()
        ...
        self.cond_projector = nn.Linear(in_features=cond_dim, out_features=self.emb_channel)

    def forward(self, x, cond, noise_labels):
...
class FlowPrecond(torch.nn.Module):
    def __init__(self, sigma_min=0, sigma_max=float('inf'), sigma_data=0.5, in_channels=2, cond_dim=27, **model_kwargs):
        super().__init__()
        self.in_channels = in_channels
        self.model = MPModel(in_channels, cond_dim=cond_dim, **model_kwargs)
...
class FlowPackerWrapper(torch.nn.Module):
    def __init__(self):
        super().__init__()
        ckpt_dict = torch.load("flowpacker/checkpoints/cluster.pth", map_location="cuda", weights_only=False)
